...
var the_filename := "res://game/World.json"
var layers : Dictionary = {}
var game_settings : Dictionary = {}

func _ready():
    import_from_file(the_filename)
    finished_init.emit()
...
func import_from_file(filename : String) -> void:
    ...
    if BroManager.ALL_DATA.has("camera_speed"):
        ...
        game_settings["camera_speed"] = BroManager.ALL_DATA["camera_speed"]
    ...
    var layers_fold = create_foldble_return_vbox(self, "layers")
    layers_fold.set_meta("is_arr", false)
    
    for layer_name in BroManager.ALL_DATA["layers"]:
        var new_layer = LAYER.instantiate()
        layers_fold.add_child(new_layer)
...
func _on_save_butt_pressed() -> void:
    for el in game_settings:
        BroManager.ALL_DATA[el] = game_settings[el]
    
    #var save_data = {}
    #save_data["layers"] = {}
    for layer_name in layers:
        BroManager.ALL_DATA["layers"][layer_name] = layers[layer_name].get_data()
    
    BroManager.save_to_json_file(BroManager.ALL_DATA, "res://game/World.json")
