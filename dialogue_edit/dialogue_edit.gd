...
func save_everything() -> Dictionary:
    ...
    var big_json_data : Dictionary = BroManager.ALL_DATA
    BroManager.ALL_DATA["layers"][layer_name]["NPC"][character_name]["NPC"]["defult_state"] = float(get_node(String(connections_queue[0]["to_node"])).state_index)
    ...
    while len(connections_queue) > 0:
        ...
        match curr_node_type:
            ...
            BroManager.DialNodeType.LOGIC:
                save_data[curr_node_ind] = get_data_from_logic(curr_node, curr_conn)
            
    #var data = big_json_data.data #json_data.stringify(json_data)
    
    print(save_data)
    
    big_json_data["layers"][layer_name]["NPC"][character_name]["NPC"]["states"] = save_data
...
func get_data_from_logic(node : GraphNode, _connections : Array[Dictionary]) -> Dictionary:
    var res_json : Dictionary = node.get_data()

    res_json["type"] = "logic"
    res_json["true_state"] = find_1st_connection_node_index_from_port_from_node_or_minus1(_connections, node.name, 0)
    res_json["false_state"] = find_1st_connection_node_index_from_port_from_node_or_minus1(_connections, node.name, 1)

    
    
    return res_json

func import_from_states_data(states_data : Dictionary, defoult_state : float, _npc_name : String, _layer_name : String):
    character_name = _npc_name
    layer_name = _layer_name
    
    var pos = Vector2(212, 88)
    ...
    while joined_nodes < len(indexes_given_dict) and len(queue) > 0:
        ...
        if curr_node.type == BroManager.DialNodeType.CHOICE:
            ...
            var output_index_true = states_data[curr_index]["true_state"]
            var output_index_false = states_data[curr_index]["false_state"]
            connect_node(curr_node_name, 0, indexes_given_dict[output_index_true].name, 0)
            connect_node(curr_node_name, 1, indexes_given_dict[output_index_false].name, 0)
            queue.append(output_index_true)
            queue.append(output_index_false)
